    ...
    <main className="min-h-screen bg-[#F7F9FC] px-4 py-10">
      <section className="mx-auto w-full max-w-6xl">
        ...
        <header className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#1F3A5F]">Admin Dashboard</h1>
        ...
        <section className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="rounded-lg border border-[#DCE3ED] bg-white px-4 py-3 shadow-sm">
            <p className="text-[11px] uppercase tracking-[0.1em] text-[#6E8196]">Active Faculty</p>
            <p className="mt-1 text-lg font-semibold text-[#1F3A5F]">42</p>
          </div>
          <div className="rounded-lg border border-[#DCE3ED] bg-white px-4 py-3 shadow-sm">
            <p className="text-[11px] uppercase tracking-[0.1em] text-[#6E8196]">Open Tickets</p>
            <p className="mt-1 text-lg font-semibold text-[#1F3A5F]">7</p>
          </div>
          <div className="rounded-lg border border-[#DCE3ED] bg-white px-4 py-3 shadow-sm">
            <p className="text-[11px] uppercase tracking-[0.1em] text-[#6E8196]">Pending Requests</p>
        ...
        <section className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
          {features.map((f) => (
            ...
            >
              ...
              <h2 className="mb-1 text-base font-semibold text-[#1F3A5F]">{f.title}</h2>
              <p className="mb-4 text-sm text-[#5A6C7D]">{f.desc}</p>
              <button className="rounded-md border border-[#4A6FA5] px-4 py-2 text-sm text-[#4A6FA5] transition hover:bg-[#4A6FA5]/10">
                {f.action}
              </button>
            </article>
          ))}
        ...
        <section className="rounded-lg border border-[#DCE3ED] bg-white p-6 shadow-sm">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="mb-1 text-lg font-semibold text-[#1F3A5F]">Add Faculty</h3>
              <p className="mb-4 text-sm text-[#5A6C7D]">
                Create a new faculty profile and assign role-specific details for scheduling.
              </p>
              <button className="rounded-md bg-[#1F3A5F] px-4 py-2 text-sm text-white transition hover:bg-[#2A4A75]">
                Add Faculty
              </button>
            </div>
          </div>
